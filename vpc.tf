...
# exercise: EKS/ECS is a commented stub, no SG resources here (see
# DESIGN.md A5 for the IAM/security model — SGs live in a separate sg.tf
# owned by the platform/network team, not bundled into the network module,
# so app teams can never touch them).
#
# ---------------------------------------------------------------------
...
#                              network-core (see stub below). The per-AZ
#                              ENIConfig CRs that bind pods to the
#                              secondary CIDR are applied here (kubectl/
#                              helm), not in network-core — they're a
#                              cluster-level construct even though the
#                              underlying CIDR association is network-level.
#
...
variable "hub_cidr" {
  ...
  # ad hoc — see DESIGN.md A1 CIDR strategy. Hub #1 in that supernet; hub #2
  # would get 10.101.0.0/16, etc. Hardcoding the literal here only because
  # the take-home says the TGW itself can be a variable; in the real repo
  # this comes from the IPAM pool data source.
...
variable "pod_secondary_cidr" {
  ...
  # see DESIGN.md A1. Deliberately NOT sized/allocated by the same IPAM
  # discipline as hub_cidr: pod IPs never cross the TGW, so every regional
  # hub can reuse this exact /16 with zero collision risk.
...
# dual Direct Connect + VPN backup (see DESIGN.md A1/A4). Its own reserved
# supernet, separate from hub_cidr's, so ground space and hub-VPC space
# can never collide as more hubs are added.
...
# attachment but — per DESIGN.md A1 — via a second, separately-owned TGW
# route table, so ground traffic and Org traffic stay segmented at the TGW
# rather than flattened into one routing domain. This variable only
...
# file. Placeholder value below — real list comes from the network account.
variable "org_cidrs" {
  type    = list(string)
  default = ["10.96.0.0/11"]
}
...
# from it via prefix delegation. Kept as its own resource/association
# because it's logically a distinct address plan from hub_cidr: allocated
# differently, reused identically across hubs, never routed over the TGW.
...
# ---------------------------------------------------------------------
# SUBNETS — 2 tiers x 3 AZs, matching NetworkDesign.jpeg exactly. No
# separate tgw-attach tier: the TGW attachment ENIs land in the private
# subnets (see DESIGN.md A1 for the trade-off — a dedicated tier gives
# cleaner blast-radius separation, added later if a security review asks).
# ---------------------------------------------------------------------
...
resource "aws_subnet" "private" {
  ...
  # DESIGN.md A1) — 251 usable IPs covers that with headroom for the TGW
  # attachment ENI and interface-endpoint ENIs. Subnets don't resize in
  # place, so this is sized for the target, not the day-1 node count.
...
resource "aws_ec2_transit_gateway_vpc_attachment" "hub" {
  ...
  # org-east-west — see DESIGN.md A1), not by anything in this VPC. That
  # segmentation lives in the network-core account's TGW config, out of
  # scope for this file.
}
...
# see NetworkDesign.jpeg. Omitted as a resource here on purpose: it's
